...
| D-061 ✓ | **Launch shape = B:** hold launch until multiplayer ships; **NYC-only soft-launch; ~3–4+ week target.** Scope: scavenger hunt + geo-locked camera collectible + multiplayer (friends + shared hunts + leaderboard) + Sign in with Apple + cartoonish gamification pass. | CEO chose B over the solo-launch-first option | 2026-06-28 |
| D-070 ✓ (building) — test-run feedback | **Clues = SHORT punchy RIDDLES, not long descriptive paragraphs.** CEO walk feedback: clues are too long / not fun; likes the history but wants it AFTER the find (the reward) or as a "learn more," not in the clue. Fix (server clue-gen): clue = a tight, playful, game-like riddle (~≤30 words / a few lines, cryptic-but-solvable, no history lesson, no name); the lore/history moves entirely to the post-find REVEAL + an optional "learn more." Difficulty = crypticness of the riddle, not length. | The core concept is landing; this is clue-style tuning | 2026-06-29 |
| D-069 ✓ (building) — test-run fix | **Virtual item must require GPS presence in the clue's geofence.** Item/camera-catch is collectible ONLY when within the find radius of the target (you actually went there). The manual "I found it!" only completes-with-item when in-zone; the give-up "Reveal this place" safety still reveals the name + advances but **grants no item** (you weren't there). Auto-find ≤50m grants the item. No-trap preserved (can always advance); item = proof of presence. Keep radius generous (Manhattan GPS). | CEO test-run: items were collectable outside the geofenced area | 2026-06-29 |
| D-068a ✓ | **Curation pipeline ran:** 490 NYC POIs pulled (7 nbhds) → labeled (category + themes + blurb). **206 approved** walk-up places, 89 "maybe" (sensitivity/borderline), 195 flagged (districts/events/stations culled). Themes: historic 260, architecture 137, art 73, iconic 72, music 52, immigrant 52, political 48, foodie 31, hidden_gem 25, scandal 20, literary 19, lgbtq 18, film_tv 17, maritime 9, **haunted 1**. Labeled set committed to `db/nyc-pois-labeled.json`. **KEY FINDING: a "Haunted" tour ISN'T viable from open-data lore (only 1 haunted place) — ghost tours need a DEDICATED folklore source or hand-curation.** Viable themed tours NOW: literary, music, lgbtq-history, architecture, immigrant-history. Next: load → Supabase `poi` table (needs poi_schema.sql + service key) → wire quest-gen to prefer the curated DB. | Sensitivity handled well (Stonewall Inn approved vs Stonewall riots flagged) | 2026-06-29 |
| D-068 ✓ (building) | **Curation pipeline (the moat):** Agent 1 pulls + interest-filters NYC POIs (7 storied Manhattan neighborhoods) → a curated dataset; Agent 2 (Claude, using its own knowledge — no API cost) labels each with neighborhood + category + **themes** (haunted/historic/literary/foodie/architecture…) + interestingness + a one-line why → loads into the `poi` table. Powers themed tours (D-056) + "more interesting places" (D-065). Runs PARALLEL to launch (enriches data, non-blocking). Follow-ons: load labeled file → Supabase (needs poi table + service key); then wire quest-gen to PREFER the curated DB (D-046 Increment 2). | CEO wants to curate places to be interesting + assignable by meaning | 2026-06-29 |
| D-067 ✓ (building) — amends D-065 | **Warmer/colder map signal → peripheral EDGE-GLOW + movement-gated debounce** (per UX-RESEARCH R3), replacing the full-screen color wash. Edges tint blue(cold)→red(hot), map center stays clear/legible outdoors; heat only refreshes after the user moves ~12m (hides Manhattan GPS jitter, feels deliberate — geocaching's trick). Keep band haptics + text label. | CEO: "go with the researcher's instincts on the map" | 2026-06-29 |
| D-066 ✓ | **Gamification research → retention roadmap** (`docs/GAMIFICATION-RESEARCH.md`). Findings: (a) shipped code is ALREADY weekly (good); retire the stale daily mechanics in `docs/GAME_DESIGN.md` (PRD is source of truth). (b) **Top non-daily retention lever = scheduled co-presence: "Quest with a friend this weekend"** (pick hunt → invite → set a day → 1 gentle reminder → shared celebration) — builds on shared hunts; post-launch priority #1. (c) Seasonal/event calendar = the return engine for completed home turf (Pokémon-GO Community-Day model). (d) Our hard-clue→reveal loop IS the variable reward — sharpen the discovery reveal, don't bolt on gacha. (e) Forgiving weekly streak (freezes, "paused not lost"). (f) Long-arc city campaign (D-064) confirmed as retention engine. (g) **Adopt a dark-pattern BAN LIST as a product value:** no guilt notifications, no manufactured FOMO, no pay-to-win, no paid loot boxes (also App-Store/FTC/PEGI-safe). Most items = post-launch retention roadmap. | CEO asked for engagement research; framed ethical/healthy-habit | 2026-06-29 |
| D-065 ✓ (building) | **UX simplify/focus pass:** (1) welcome = just "Start a Quest" + "Start a Quest with Friends", NO teaser facts; (2) **remove the Quest Setup screen entirely** — no walk/bike, no location picker, no difficulty selector → tap-to-start uses current GPS + internal defaults (walk, **hard** difficulty, tight length); friends button → shared hunt + invite; (3) clue = its own separate tappable card, map also interactive; (4) warmer/colder rendered as the **MAP tinting red(hot)/blue(cold)**; (5) clues always **hard→extremely-hard** (server default → hard); (6) **more interesting locations** (bias POI selection to storied/quirky/surprising over generic). | CEO UX direction after seeing it on device: focus on clues + the hunt, kill setup friction | 2026-06-29 |
| D-064 (design) | **Riddle difficulty + bank + city campaign.** **Layer A (lean, launch-able):** a difficulty setting (Easy/Tricky/Hard/Nearly-Impossible) → AI writes the clue at that level. **Layer B (post-launch content):** a curated **~500-riddle bank** in the POI DB (multiple riddles/place, difficulty + **friend-only** flag), generated via AI-drafts→human-curation; + a **"do the city" campaign** (meta-progression: % of NYC solved across all areas). Rides on the (deferred) populated POI DB. CPO rec: ship Layer A at launch; Layer B = flagship post-launch "Season 1" content. | CEO: varying difficulty, ~500 riddles, some friend-only, an overall city quest, some nearly impossible | 2026-06-28 |
| D-063 ✓ (building) | **Pre-launch hardening pass:** (1) rate-limit the cost-incurring endpoints (/quest, /resolve-place call Claude/Google — per-IP limit → 429, prevents bill runaway/abuse once public); (2) **fix email exposure** — replace the broad cross-user profiles read with a `public_profiles` VIEW exposing only id/display_name/avatar (email/points stay own-row); app reads the view; (3) validation/CORS/error-leak review. | CEO: do the harden pass; closes the D-061 launch privacy/security gap | 2026-06-28 |
| D-062 ✓ (building, app pass) | **More cartoonish / more gamified** — push the visual style further toward a playful cartoon game UI (chunky rounded "sticker" shapes, bold outlines, more saturated cartoon palette, juicy animations, badge/sticker collectibles). NOTE: a true illustrated/mascot look needs a designer + custom art (a cost decision); styling-based cartoonish is what we do without that. | CEO: "more gamification, more cartoonish" (further than the PoGo recolor D-054) | 2026-06-28 |
| D-060 → SUPERSEDED by D-061 | **Friends / multiplayer** — social graph (Supabase friendships/invites), shared/competitive hunts. The largest pillar; has a cold-start problem (no friends on the app yet) + weeks of build. Async-leaderboard groundwork exists (D-044). CPO rec: build AFTER a real-tester signal that solo hunts are fun. | CEO: "do it with friends" | 2026-06-28 |
