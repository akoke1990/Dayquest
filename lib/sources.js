...
// Safety ceiling on lore length. Was 600 — which cut most Wikipedia intros
// mid-story and made the post-find "read the history" reward end in an
// ellipsis (field-tester report). Candidates now carry the (nearly) full
// snippet; the LLM-prompt token budget is protected separately by promptClip()
// in lib/quest.js, so this only guards against pathological multi-page intros.
...
export async function fromPlaces(lat, lng, { radius_m = 1500 } = {}) {
  ...
  try {
    ...
    console.warn(JSON.stringify({ level: "warn", event: "place_source_failed", provider: "google_places" }));
    return [];
  }
}
...
async function fromPoiDb(lat, lng, opts = {}) {
  ...
  try {
    ...
    console.warn(JSON.stringify({ level: "warn", event: "place_source_failed", provider: "poi_db" }));
    return [];
  }
  return rows
    .map((r) => curatedRowToCandidate(r, lat, lng))
    .filter((c) => candidateInRange(c, radius_m));
}

// Shared mapper: curated row (file or Supabase) → candidate. Carries the
// curation `tags` + `category` through so themed quests (type=ghosts etc.) can
// filter PRECISELY on human labels instead of guessing from prose.
function curatedRowToCandidate(r, lat, lng) {
  return {
...
function candidateInRange(c, radius_m) {
  return (
...
// --- Curated FILE source (committed datasets, zero-config) -------------------
// Loads the curated datasets COMMITTED to db/*.json (the labeler's output
// format: rows with status/category/tags/blurb) and serves the APPROVED rows
// near the origin. This is what makes curated + themed quests work in
// production WITHOUT Supabase — the data ships with the deploy. Files are read
// once and cached for the process lifetime (they only change with a deploy).
// Listed before the live web sources so curated rows win name-dedupe.
let _curatedFileRows = null;
function loadCuratedFileRows() {
  if (_curatedFileRows) return _curatedFileRows;
  _curatedFileRows = [];
  try {
    ...
    for (const f of readdirSync(dir)) {
      ...
      try {
        ...
        if (Array.isArray(rows)) {
          for (const r of rows) {
            if (r && r.status === "approved" && r.name) _curatedFileRows.push(r);
          }
        }
      } catch {
        /* one bad file never breaks the source */
      }
    }
  } catch {
    /* no db/ dir — source is a no-op */
  }
  return _curatedFileRows;
}

async function fromCuratedFile(lat, lng, opts = {}) {
  const radius_m = opts.radius_m || 1500;
  return loadCuratedFileRows()
    .map((r) => curatedRowToCandidate(r, lat, lng))
    .filter((c) => candidateInRange(c, radius_m));
}
...
export async function gatherCandidates(lat, lng, opts = {}) {
  ...
  // Curated sources first so their lore-rich rows win name-dedupe over raw web
  // hits: committed files (always on), then Supabase (no-op unless configured).
  const sources = [fromCuratedFile, fromPoiDb, fromWikipedia, fromOpenStreetMap, fromPlaces];
  ...
  for (const r of results) {
    if (r.status !== "fulfilled") {
      console.warn(JSON.stringify({ level: "warn", event: "place_source_failed", provider: "external" }));
      continue;
    }
  ...
  // SLOT RESERVATION under the prompt cap. Curated rows are scarce and
  // human-approved — a dense live-source area must never crowd them out of the
  // LLM's pool. Ghost hunts go further: EVERY in-range creepy row keeps a slot
  // (the hunt is built exclusively from them, so each one dropped here is a
  // stop the hunt can't use). Priority rows first, then nearest fill the rest.
  const priority =
    opts.type === "ghosts"
      ? candidates.filter(isCreepy)
      : candidates.filter((c) => Array.isArray(c.tags) && c.tags.length > 0).slice(0, 30);
  const prioritySet = new Set(priority);
  const rest = candidates.filter((c) => !prioritySet.has(c));
  const kept = [...priority, ...rest].slice(0, Math.max(maxCandidates, priority.length));
  kept.sort((a, b) => a.distance_m - b.distance_m);
  return kept;
}

// Curation tags that make a candidate GHOST-HUNT eligible. Lives here (with the
// candidate shape) so both the pool-cap reservation above and buildQuest's
// theme narrowing share one definition.
const GHOST_TAGS = new Set(["haunted", "scandal_crime", "cemetery", "macabre"]);
export function isCreepy(c) {
  return Array.isArray(c.tags) && c.tags.some((t) => GHOST_TAGS.has(t));
}
