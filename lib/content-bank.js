...
function candidateToken(candidate) {
  ...
  if (!/^dq-nyc-\d{3}$/.test(token)) throw new Error(`Invalid candidate research_id: ${candidate?.research_id}`);
  return token;
}

function mergeSourceCandidates(bank, artifact) {
  if (artifact == null) return bank;
  if (artifact.artifact_version !== "1.0.0" || artifact.site_id !== "nyc" || !Array.isArray(artifact.candidates)) {
  ...
  for (const candidate of artifact.candidates) {
    ...
    if (seenPlaceIds.has(candidate.place_id)) throw new Error(`Ambiguous candidate place mapping: ${candidate.place_id}`);
    seenResearchIds.add(token);
    seenPlaceIds.add(candidate.place_id);

    const place = placesById.get(candidate.place_id);
    if (!place) throw new Error(`Candidate ${candidate.research_id} references unknown approved place ${candidate.place_id}`);
    if (place.name !== candidate.matched_catalog_name) {
      throw new Error(`Candidate ${candidate.research_id} catalog name does not exactly match ${place.name}`);
    }
    const source = candidate.source;
    if (!source?.url || !source?.excerpt || source.provider !== "wikipedia"
      || `place:${source.provider}:${String(source.external_id).toLowerCase()}` !== place.id) {
    ...
    const sourceId = `source:${source.provider}:${source.external_id}:research-${token}`;
    place.sources.push({
      source_id: sourceId,
    ...
    const evidenceIds = candidate.evidence.map((item) => {
      const evidenceId = `evidence:${source.provider}-${source.external_id}:${token}-${item.local_id}`;
    ...
    bank.clue_packages.push({
      id: `clue:nyc:${source.provider}-${source.external_id}:${token}`,
      record_version: 1,
