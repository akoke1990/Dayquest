...
export async function fetchSharedHunt(id) {
  ...
  try {
    ...
    const { data, error } = await client
      ...
      .maybeSingle();
    if (error) {
      console.error(JSON.stringify({ level: "error", event: "shared_hunt_store_failed", operation: "select" }));
      return null;
    }
...
export async function upsertSharedHunt(id, quest, { area, mode, size } = {}) {
  ...
  try {
    ...
    const { error } = await client
      ...
      .abortSignal(AbortSignal.timeout(SUPABASE_TIMEOUT_MS));
    if (error) {
      console.error(JSON.stringify({ level: "error", event: "shared_hunt_store_failed", operation: "upsert" }));
      return false;
    }
    return true;
  } catch {
