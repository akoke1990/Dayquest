...
import { gatherCandidates, haversine_m, candidateToPoiRow, isCreepy } from "./sources.js";
...
function normType(type) {
  return type === "historic" || type === "barcrawl" || type === "ghosts" ? type : null;
}
...
place.) If true bars are sparse, fall back to the liveliest social venues.`;
    case "ghosts":
      return `\n\nTHEME — GHOSTS & CREEPY: Build a spooky hunt. STRONGLY prefer
places with documented ghost legends, macabre history, old cemeteries, crime
lore, and eerie oddities. Voice: playful-spooky campfire-story energy — delicious
shivers, NOT horror. HARD RULES for this theme: present legends AS legends
("said to be haunted by...", "locals swear..."); NEVER invent a haunting or a
death that isn't in the place's snippet; NEVER make light of real suffering —
the fun is the mystery and the goosebumps, not the tragedy. Let the intro set a
delightfully eerie mood and save each place's full dark story for the post-find
reveal.`;
    default:
      return "";
  }
...
// Prompt-side lore clip. Candidates now carry the FULL sourced snippet (so the
// served quest's post-find reveal can show the whole story); the clip lives
// HERE so only the LLM prompt pays the token cost of 60 candidates.
function promptClip(text, max = 600) {
  if (!text) return "";
...
function buildUserPrompt(candidates, label, maxStops = 3) {
  const lines = candidates.map(
    (c, i) =>
      ...
      `     ${c.lore ? promptClip(c.lore) : "(no history snippet available)"}`
  );
...
export async function buildQuest(lat, lng, label = `${lat}, ${lng}`, opts = {}) {
  ...
  const gathered = await gatherCandidates(lat, lng, { radius_m, area: label, type });
  ...
  if (poidbConfigured) {
    ...
    if (rows.length) {
      upsertPois(rows)
        .then((r) => console.log(JSON.stringify({ level: "info", event: "poi_upsert_complete", count: r.upserted })))
        .catch(() => console.warn(JSON.stringify({ level: "warn", event: "poi_upsert_failed" })));
    }
  }
  ...
  if (candidates.length < 3) {
    const wider = await gatherCandidates(lat, lng, { radius_m: Math.max(radius_m * 2, 6000), area: label, type });
  ...
  // THEME=GHOSTS: a ghost hunt is built ONLY from curated creepy places (tagged
  // haunted / scandal_crime / cemetery by the labeler) — never bluffed from
  // untagged web hits. Enough of them → the pool narrows to exactly those. Too
  // few → fail honestly with a friendly message instead of a bait-and-switch
  // "ghost" hunt of ordinary parks. (Live areas today: wherever curated data
  // ships in db/*.json.)
  if (type === "ghosts") {
    const creepy = candidates.filter(isCreepy);
    if (creepy.length >= 3) {
      candidates = creepy;
    } else {
      const e = new Error(
        "No spooky places are mapped near you yet — ghost hunts are live in select areas for now. Try a Surprise Me quest instead!"
      );
      e.code = "TOO_FEW";
      throw e;
    }
