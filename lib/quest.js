...
// identical across modes (quick 3 / explore 5).
//
// Fields per preset:
...
function normSize(size) {
  return size === "explore" ? "explore" : DEFAULT_SIZE;
}
...
function templatedQuest(lat, lng, label, candidates, preset = MODE_SIZE_PRESETS[DEFAULT_MODE][DEFAULT_SIZE]) {
  ...
  const stops = chosen.map((place, i) => {
    ...
    return {
      ...
      clue: previewClue(bucket),
      hint: previewHint(bucket),
