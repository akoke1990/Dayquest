...
const MAX_LOOP_M = 1500; // ceiling on total walked path (sum of consecutive legs)
...
// total loop ≤ ceiling. Returns { ok, legs, total, minLeg }.
function spreadReport(stops) {
  const legs = legDistances(stops);
  const total = legs.reduce((s, d) => s + d, 0);
  const minLeg = legs.length ? Math.min(...legs) : 0;
  const ok = minLeg >= MIN_STOP_SEP_M && total <= MAX_LOOP_M;
...
- Pick exactly 3 that form a fun, walkable loop and offer VARIETY: at least 2
  distinct place-types/micro-contexts (e.g. a park monument → a street-level
  façade → a tucked-away courtyard) — don't pick three of the same kind.
- SPREAD / WALKABILITY (this is load-bearing — a quest must feel like a walk,
  ...
  apart, and NEVER closer than ~250m. The total walking loop (sum of the two
  consecutive legs) should be ~0.8–1.5km — about a 30-minute walk. Use the
  given coordinates/distances to judge this; reject clusters that ring a single
  plaza.
- HERO FIRST: choose the single most striking/iconic place as stop 1 (the
  payoff a first-time explorer hits first), then order the remaining two into a
  sensible walking sequence.
...
export async function buildQuest(lat, lng, label = `${lat}, ${lng}`) {
  ...
  if (preview) return templatedQuest(lat, lng, label, candidates);
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    const rep = spreadReport(candidate.stops);
    if (rep.ok) {
      best = { candidate, rep };
      break;
    }
    const better =
      !best ||
      (rep.total <= MAX_LOOP_M && best.rep.total > MAX_LOOP_M) ||
      (rep.total <= MAX_LOOP_M === best.rep.total <= MAX_LOOP_M &&
...
// Falls back to ignoring the floor only if too few candidates clear it.
function pickVaried(candidates) {
  const usable = candidates.filter((c) => !isSensitive(c));
  const pool = usable.length >= 3 ? usable : candidates;
  const picked = [];
  ...
  // Pass 3: graceful fallback — if the floor left us short (sparse area), top
  // up with the nearest unused places so we still return a quest.
  for (const c of pool) {
    if (picked.length >= 3) break;
...
function templatedQuest(lat, lng, label, candidates) {
  const chosen = orderForWalk(pickVaried(candidates), lat, lng);
  ...
  const rep = spreadReport(stops);
  return {
    theme: "A Little Local Wander",
    intro: "Three nearby spots, a short walk, and a few stories. Ready to explore?",
    origin: { lat, lng, label },
    stops,
    meta: {
      mode: "preview",
