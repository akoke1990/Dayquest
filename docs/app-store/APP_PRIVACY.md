...
| Other User Content | Yes | **Conservative: Yes** | Functionality; Analytics | Ratings/text/stop flags/context go to `/feedback` with install ID. Structured content failure reports go to `/content-failure` and the service-role-only Supabase queue with reason, curated place/content-version IDs, priority/action/accessibility/status, and a server request ID. They contain no raw GPS coordinates, routes, photos, clue text/answer, email, IP, user/account ID, install ID, or free-form field. | Disclose separately; approve retention/deletion process and queue operations. |
