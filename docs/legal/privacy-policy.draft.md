...
- **Content failure reports:** When a user reports a curated stop as unsafe, blocked/closed, inaccessible, missing, or incorrect, the API records only the reason, curated place/content-version identifiers, queue priority/action/accessibility/status, and a server-generated request ID. The report contains no raw GPS coordinates, route, photo, clue text or answer, email, IP, user ID, install ID, or free-form field. Production does not provide a replacement unless this structured report is durably stored in a service-role-only Supabase queue; a timestamped local operational JSONL copy is also written.
